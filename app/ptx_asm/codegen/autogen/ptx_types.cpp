...
static std::unordered_map<std::string, setpcmpQl> StrTosetpcmpQl{
    {"", setpcmpQl::UNDEFINED}, {"eq", setpcmpQl::Eq},   {"ne", setpcmpQl::Ne},   {"lt", setpcmpQl::Lt},
    {"le", setpcmpQl::Le},      {"gt", setpcmpQl::Gt},   {"ge", setpcmpQl::Ge},   {"lo", setpcmpQl::Lo},
    {"ls", setpcmpQl::Ls},      {"hi", setpcmpQl::Hi},   {"hs", setpcmpQl::Hs},   {"equ", setpcmpQl::Equ},
    {"neu", setpcmpQl::Neu},    {"ltu", setpcmpQl::Ltu}, {"leu", setpcmpQl::Leu}, {"gtu", setpcmpQl::Gtu},
    {"geu", setpcmpQl::Geu},    {"num", setpcmpQl::Num}, {"nan", setpcmpQl::Nan},
};
...
registerOperand FromString(const std::string& str)
{
    ...
    {
        if (match[1].matched)
            res.type = FromString<registerType>(match[1].str());
        if (match[2].matched)
            res.reg_id = FromString<uint32_t>(match[2].str());
    }
    else
    {
...
addressOperand FromString(const std::string& str)
{
    ...
    {
        if (match[1].matched)
            res.reg = FromString<registerOperand>(match[1].str());
        if (match[2].matched)
            res.symbol = FromString<symbolOperand>(match[2].str());
        if (match[3].matched)
            res.imm = FromString<immediateOperand>(match[3].str());
    }
    else
    {
