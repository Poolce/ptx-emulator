...
std::shared_ptr<regInstruction> regInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(\.reg\s\.([fsub]8|[fsub]16|[fsub]32|[fsub]64|pred)\s%(f|fd|p|r|rd|spr)<([0-9]+)>;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<regInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<dataType>(match[1].str()); }
            if (match[2].matched) { instr->reg_ = FromString<registerType>(match[2].str()); }
            if (match[3].matched) { instr->count_ = FromString<uint32_t>(match[3].str()); }
    } else {
...
std::shared_ptr<sharedInstruction> sharedInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(\.shared\s\.align\s([0-9]+)\s*\.([fsub]8|[fsub]16|[fsub]32|[fsub]64|pred)\s([A-z0-9_]+)\[([0-9]+)\];)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<sharedInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->align_ = FromString<uint32_t>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<dataType>(match[2].str()); }
            if (match[3].matched) { instr->symbol_ = FromString<symbolOperand>(match[3].str()); }
            if (match[4].matched) { instr->count_ = FromString<uint32_t>(match[4].str()); }
    } else {
...
std::shared_ptr<pragmaInstruction> pragmaInstruction::Make(const std::string& line) {
    ...
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->val_ = FromString<pragmavalQl>(match[1].str()); }
    } else {
...
std::shared_ptr<absInstruction> absInstruction::Make(const std::string& line) {
    ...
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localAbsDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src_ = FromString<registerOperand>(match[3].str()); }
    } else {
...
std::shared_ptr<cvtaInstruction> cvtaInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(cvta\.(to)?\.?(const|global|local|param|param::entry|shared|shared::cta|shared::cluster)\.(u32|u64)\s(%[a-z]+[0-9]*,?)\s(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<cvtaInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->to_ = FromString<bool>(match[1].str()); }
            if (match[2].matched) { instr->space_ = FromString<cvtaspaceQl>(match[2].str()); }
            if (match[3].matched) { instr->data_ = FromString<localCvtaDataType>(match[3].str()); }
            if (match[4].matched) { instr->dst_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src_ = FromString<registerOperand>(match[5].str()); }
    } else {
...
std::shared_ptr<setpInstruction> setpInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(setp\.(eq|ne|lt|le|gt|ge|lo|ls|hi|hs|equ|neu|ltu|leu|gtu|geu|num|nan)?\.?(and|or|xor)?\.?(b16|b32|b64|u16|u32|u64|s16|s32|s64|f32|f64)?\s(%[a-z]+[0-9]*,?)\s(%[a-z]+[0-9]*,?)\s(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<setpInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->cmp_ = FromString<setpcmpQl>(match[1].str()); }
            if (match[2].matched) { instr->bool_ = FromString<setpboolQl>(match[2].str()); }
            if (match[3].matched) { instr->data_ = FromString<localSetpDataType>(match[3].str()); }
            if (match[4].matched) { instr->dst_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src1_ = FromString<registerOperand>(match[5].str()); }
            if (match[6].matched) { instr->src2_ = FromString<registerOperand>(match[6].str()); }
            if (match[7].matched) { instr->imm_ = FromString<immediateOperand>(match[7].str()); }
    } else {
...
std::shared_ptr<addInstruction> addInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(add\.(u16|u32|u64|s16|s32|s64|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<addInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localAddDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src1_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src2_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->imm_ = FromString<immediateOperand>(match[5].str()); }
    } else {
...
std::shared_ptr<movInstruction> movInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(mov\.(pred|b16|b32|b64|u16|u32|u64|s16|s32|s64|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(%[a-z]+\.?[xyz]?)?(0?x?[\-A-Fa-f0-9]+)?([A-z0-9_]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<movInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localMovDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->spr_ = FromString<sprRegisterOperand>(match[4].str()); }
            if (match[5].matched) { instr->imm_ = FromString<immediateOperand>(match[5].str()); }
            if (match[6].matched) { instr->symbol_ = FromString<symbolOperand>(match[6].str()); }
    } else {
...
std::shared_ptr<shlInstruction> shlInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(shl\.(b16|b32|b64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<shlInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localShlDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->imm_ = FromString<immediateOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<andInstruction> andInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(and\.(pred|b16|b32|b64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<andInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localAndDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src1_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src2_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->imm_ = FromString<immediateOperand>(match[5].str()); }
...
std::shared_ptr<mulInstruction> mulInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(mul\.?(hi|lo|wide)?\.(f16|f32|f64|u16|u32|u64|s16|s32|s64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<mulInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->mode_ = FromString<mulmodeQl>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localMulDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src1_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src2_ = FromString<registerOperand>(match[5].str()); }
            if (match[6].matched) { instr->imm_ = FromString<immediateOperand>(match[6].str()); }
    } else {
...
std::shared_ptr<ex2Instruction> ex2Instruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(ex2\.approx(\.ftz)\.(f16|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<ex2Instruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->ftz_ = FromString<bool>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localEx2DataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src_ = FromString<registerOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<barInstruction> barInstruction::Make(const std::string& line) {
    ...
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->cta_ = FromString<bool>(match[1].str()); }
            if (match[2].matched) { instr->mode_ = FromString<barmodeQl>(match[2].str()); }
            if (match[3].matched) { instr->id_ = FromString<uint32_t>(match[3].str()); }
    } else {
...
std::shared_ptr<stInstruction> stInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(st\.(const|global|local|param|param::entry|param::func|shared|shared::cta|shared::cluster)?\.(b8|b16|b32|b64|u8|u16|u32|u64|s8|s16|s32|s64|f32|f64)?\s*([\[\]A-Za-z0-9_\+\-%]+),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<stInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->space_ = FromString<stspaceQl>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localStDataType>(match[2].str()); }
            if (match[3].matched) { instr->addr_ = FromString<addressOperand>(match[3].str()); }
            if (match[4].matched) { instr->src_ = FromString<registerOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<fmaInstruction> fmaInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(fma\.(rn|rz|rm|rp)\.(f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?,\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<fmaInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->mode_ = FromString<fmamodeQl>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localFmaDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src1_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src2_ = FromString<registerOperand>(match[5].str()); }
            if (match[6].matched) { instr->imm1_ = FromString<immediateOperand>(match[6].str()); }
            if (match[7].matched) { instr->src3_ = FromString<registerOperand>(match[7].str()); }
            if (match[8].matched) { instr->imm2_ = FromString<immediateOperand>(match[8].str()); }
    } else {
...
std::shared_ptr<negInstruction> negInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(neg\.(s16|s32|s64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<negInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localNegDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->imm_ = FromString<immediateOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<subInstruction> subInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(sub\.(f32|s64|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<subInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localSubDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src1_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src2_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->imm_ = FromString<immediateOperand>(match[5].str()); }
...
std::shared_ptr<braInstruction> braInstruction::Make(const std::string& line) {
    ...
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->prd_ = FromString<registerOperand>(match[1].str()); }
            if (match[2].matched) { instr->uni_ = FromString<bool>(match[2].str()); }
            if (match[3].matched) { instr->sym_ = FromString<symbolOperand>(match[3].str()); }
    } else {
...
std::shared_ptr<labelInstruction> labelInstruction::Make(const std::string& line) {
    ...
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->sym_ = FromString<symbolOperand>(match[1].str()); }
    } else {
...
std::shared_ptr<ldInstruction> ldInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(ld\.(const|global|local|param|param::entry|param::func|shared|shared::cta|shared::cluster)?\.(b8|b16|b32|b64|u8|u16|u32|u64|s8|s16|s32|s64|f32|f64)?\s*(%[a-z]+[0-9]*,?),\s*([\[\]A-Za-z0-9_\+\-%]+);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<ldInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->space_ = FromString<ldspaceQl>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localLdDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->addr_ = FromString<addressOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<rcpInstruction> rcpInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(rcp\.approx(\.ftz)\.(f16|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<rcpInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->ftz_ = FromString<bool>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localRcpDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src_ = FromString<registerOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<tanhInstruction> tanhInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(tanh\.approx(\.ftz)\.(f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<tanhInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->ftz_ = FromString<bool>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localTanhDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src_ = FromString<registerOperand>(match[4].str()); }
    } else {
...
std::shared_ptr<madInstruction> madInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(mad\.(hi|lo|wide)\.(u16|u32|u64|s16|s32|s64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<madInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->mode_ = FromString<madmodeQl>(match[1].str()); }
            if (match[2].matched) { instr->data_ = FromString<localMadDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src1_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src2_ = FromString<registerOperand>(match[5].str()); }
            if (match[6].matched) { instr->src3_ = FromString<registerOperand>(match[6].str()); }
    } else {
...
std::shared_ptr<copysignInstruction> copysignInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(copysign\.(f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?;)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<copysignInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localCopysignDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src1_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->src2_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->imm_ = FromString<immediateOperand>(match[5].str()); }
...
std::shared_ptr<selpInstruction> selpInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(selp\.(b16|b32|b64|u16|u32|u64|s16|s32|s64|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?,\s*(%[a-z]+[0-9]*,?)?(0?x?[\-A-Fa-f0-9]+)?,\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<selpInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->data_ = FromString<localSelpDataType>(match[1].str()); }
            if (match[2].matched) { instr->dst_ = FromString<registerOperand>(match[2].str()); }
            if (match[3].matched) { instr->src1_ = FromString<registerOperand>(match[3].str()); }
            if (match[4].matched) { instr->imm1_ = FromString<immediateOperand>(match[4].str()); }
            if (match[5].matched) { instr->src2_ = FromString<registerOperand>(match[5].str()); }
            if (match[6].matched) { instr->imm2_ = FromString<immediateOperand>(match[6].str()); }
            if (match[7].matched) { instr->src3_ = FromString<registerOperand>(match[7].str()); }
    } else {
...
std::shared_ptr<cvtInstruction> cvtInstruction::Make(const std::string& line) {
    constexpr std::string_view Pattern = R"ptx(cvt\.?(rtz|rm|rz|rn|rp)?\.(u8|u16|u32|u64|s8|s16|s32|s64|f16|f32|f64)\.(u8|u16|u32|u64|s8|s16|s32|s64|f16|f32|f64)\s*(%[a-z]+[0-9]*,?),\s*(%[a-z]+[0-9]*,?);)ptx";
    static std::regex re(Pattern.data(), std::regex::ECMAScript | std::regex::optimize);
    std::smatch match;
    auto instr = std::make_shared<cvtInstruction>();
    if (std::regex_match(line, match, re)) {
            if (match[1].matched) { instr->mode_ = FromString<cvtmodeQl>(match[1].str()); }
            if (match[2].matched) { instr->src_data_ = FromString<localCvtSrcDataType>(match[2].str()); }
            if (match[3].matched) { instr->dst_data_ = FromString<localCvtDstDataType>(match[3].str()); }
            if (match[4].matched) { instr->dst_ = FromString<registerOperand>(match[4].str()); }
            if (match[5].matched) { instr->src_ = FromString<registerOperand>(match[5].str()); }
    } else {
