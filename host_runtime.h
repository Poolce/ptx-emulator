 ...
 */
    ...
    #if defined(_MSC_VER)
        #pragma message(                                                                                               \
            "crt/device_functions.h is an internal header file and must not be used directly.  Please use cuda_runtime_api.h or cuda_runtime.h instead.")
    #else
        #warning                                                                                                       \
            "crt/device_functions.h is an internal header file and must not be used directly.  Please use cuda_runtime_api.h or cuda_runtime.h instead."
    #endif
    ...
    #define __text__
    #define __surf__
    #define __name__shadow_var(c, cpp)   #c
    #define __name__text_var(c, cpp)     #cpp
    #define __host__shadow_var(c, cpp)   cpp
    #define __text_var(c, cpp)           cpp
    #define __device_fun(fun)            #fun
    #define __device_var(var)            #var
    #define __device__text_var(c, cpp)   #c
    #define __device__shadow_var(c, cpp) #c
...
static inline T* __cudaAddressOf(T& val)
{
    ...
    #define __cudaRegisterGlobalTexture(handle, tex, dim, norm, ext)                                                   \
        __cudaRegisterTexture(handle,                                                                                  \
                              (const struct textureReference*)&tex,                                                    \
                              (const void**)(void*)__device##tex,                                                      \
                              __name##tex,                                                                             \
                              dim,                                                                                     \
                              norm,                                                                                    \
                              ext)
    #define __cudaRegisterGlobalSurface(handle, surf, dim, ext)                                                        \
        __cudaRegisterSurface(handle,                                                                                  \
                              (const struct surfaceReference*)&surf,                                                   \
                              (const void**)(void*)__device##surf,                                                     \
                              __name##surf,                                                                            \
                              dim,                                                                                     \
                              ext)
    #define __cudaRegisterEntry(handle, funptr, fun, thread_limit)                                                     \
        __cudaRegisterFunction(handle,                                                                                 \
                               (const char*)funptr,                                                                    \
                               (char*)__device_fun(fun),                                                               \
                               #fun,                                                                                   \
                               -1,                                                                                     \
                               (uint3*)0,                                                                              \
                               (uint3*)0,                                                                              \
...
extern "C" cudaError_t CUDARTAPI __cudaPopCallConfiguration(dim3* gridDim,
    ...
    #define __cudaSetupArg(arg, offset)                                                                                \
        __args_arr[__args_idx] = (void*)__cudaAddressOf(arg);                                                          \
        ++__args_idx

    #define __cudaSetupArgSimple(arg, offset)                                                                          \
        __args_arr[__args_idx] = (void*)(char*)&arg;                                                                   \
        ++__args_idx
        ...
        #define __cudaLaunch(fun, isTileKernel)                                                                        \
            {                                                                                                          \
                ...
                {                                                                                                      \
                    (void)                                                                                             \
                        cudaLaunchKernel(fun, __gridDim, __blockDim, &__args_arr[__args_idx], __sharedMem, __stream);  \
                }                                                                                                      \
                else                                                                                                   \
                {                                                                                                      \
        ...
        #define __cudaLaunch(fun, isTileKernel)                                                                        \
            {                                                                                                          \
                ...
                if (__args_idx == 0)                                                                                   \
                {                                                                                                      \
                    (void)__cudaLaunchKernel_helper(__handle,                                                          \
                                                    __gridDim,                                                         \
                                                    __blockDim,                                                        \
                                                    &__args_arr[__args_idx],                                           \
                                                    __sharedMem,                                                       \
                                                    __stream);                                                         \
                }                                                                                                      \
                else                                                                                                   \
...
extern "C"
{
    ...
    static const unsigned char __module_id_str[] __attribute__((section("__NV_CUDA,__nv_module_id"))) =
        __TO_STRING(__NV_MODULE_ID);
    #else
    static const unsigned char __module_id_str[] __attribute__((section("__nv_module_id"))) =
        __TO_STRING(__NV_MODULE_ID);
    #endif
    ...
    #define ____cudaRegisterLinkedBinary(X)                                                                            \
        {                                                                                                              \
            __REGISTERFUNCNAME(__NV_MODULE_ID)                                                                         \
            ((void (*)(void**))(X),                                                                                    \
...
extern "C"
{
    ...
    extern void CUDARTAPI __cudaRegisterSurface(void** fatCubinHandle,
