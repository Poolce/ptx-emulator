...
Применение универсальных инструментов синтаксического анализа (ANTLR, PEG-грамматики)
для анализа PTX избыточно: грамматика PTX не содержит неоднозначностей,
набор мнемоник и директив фиксирован спецификацией ISA~7.x~\cite{PTXISA}, а выполнение
...
Анализатор реализует конечный автомат~\cite{Martynenko2004}
с состояниями $Q = \{q_{\mathrm{init}}, q_{\mathrm{func}}, q_{\mathrm{block}}\}$
и переходами:
\begin{itemize}
...
Трёх состояний достаточно: PTX запрещает вложенные определения функций,
а базовые блоки внутри тела строго не пересекаются и упорядочены по тексту~---
в отличие от языков общего назначения с произвольной вложенностью областей
видимости, требующей магазинного автомата.
Разбор выполняется за $O(N)$ по числу строк без возвратов; результат~---
объект \texttt{PtxModule} с таблицей инструкций, графом базовых блоков
и таблицей символов для разрешения переходов.

\subsection{Реализация анализатора}

\subsubsection{Разбор объявлений функций и параметров}

Заголовок функции разбирается регулярным выражением~\cite{Thompson1968, Cox2007, Martynenko2004} (файл \texttt{module.cpp}):
...
связывающей синтаксический анализатор с исполнителем управляющего потока. Метка вида
...
\begin{lstlisting}[language=C++, breaklines=true, breakatwhitespace=false, caption={Фрагмент автогенерированного instructions.cpp для fma}]
...
Классификация каждого операнда производится единожды при разборе~\cite{Martynenko2004},
а не при каждом исполнении инструкции:
...
как индекс в глобальном векторе модуля.
При исполнении \texttt{Module::GetInstruction(pc)} возвращает
\texttt{const Instruction\&} за $O(1)$ через прямую индексацию вектора.
Тот же PC используется как ключ в записях \texttt{profiling.txt},
