...
\begin{table}[h]
  ...
  \begin{tabular}{cp{4.5cm}p{9cm}}
    ...
    & & $\quad\quad \{PC_C,\texttt{0x0000FFFF},T\},$ \\
    & & $\quad\quad \{PC_B,\texttt{0x0000FF00},F\}]$ \\
    \midrule
    3 & Исполнение $\mathrm{bb\_A}$ ($M = \texttt{0x000000FF}$)
      & $S$ без изменений \\
    \midrule
    4 & PC = $PC_B$ (цикл реконвергенции)
      & Снимаем $\{PC_B, \texttt{0x0000FF00}, F\}$: \\
    & & $M \leftarrow \texttt{0x0000FF00}$, \\
    & & $S = [\{100,M_0,T\},\{PC_D,M_{\mathrm{not}},F\},$ \\
    & & $\quad\{PC_C,\texttt{0x0000FFFF},T\}]$ \\
    \midrule
    5 & Исполнение $\mathrm{bb\_B}$ ($M = \texttt{0x0000FF00}$)
      & $S$ без изменений \\
    \midrule
    6 & PC = $PC_C$ (цикл реконвергенции)
