...
Фаза создания объектов (разбор) и фаза исполнения (горячий цикл)
разделены: анализатор проверяет синтаксис, исполнитель реализует семантику.

\subsubsection{Генерация кода классов инструкций}

Скрипт \texttt{autogen.py} читает \texttt{instructions.json}
и генерирует \texttt{instructions.h} и \texttt{instructions.cpp}
с классом для каждой мнемоники.
...
Аналогичный принцип применяется в системе метрик профилировщика (Глава~7):
единый JSON-файл управляет генерацией кода сразу нескольких компонентов,
что исключает рассинхронизацию.
...
Преобразование выполняется через \texttt{std::bit\_cast} (C++20).
...
Целые числа преобразуются посредством \texttt{static\_cast} через беззнаковый
промежуточный тип~--- бит-паттерн знаковых значений в \texttt{uint64\_t}
сохраняется. Числа с плавающей запятой сериализуются через \texttt{memcpy}.

Хранение всех значений в \texttt{uint64\_t} исключает шаблонный
параметр по типу из структуры \texttt{ThreadContext} и сокращает
объём кода регистрового файла без потери точности.
...
\end{equation}
где \texttt{imm\_offset~= 8} извлекается синтаксическим анализатором из адресного выражения.
...
Реальный GPU воспроизводит эти граничные случаи аппаратно согласно
PTX-спецификации, поэтому эмулятор обрабатывает их явно~--- иначе
результаты расходились бы при корректном поведении на типичных путях.
...
\texttt{.rd}) разбираются синтаксическим анализатором и сохраняются в объекте инструкции.
...
\begin{equation}
  R'[t][\mathtt{s32}][d] = \mathrm{round}_n(R[t][\mathtt{f32}][a]),
  \quad \forall t \in \activeset(M),
\end{equation}
где $\mathrm{round}_n$~--- округление к ближайшему целому; при равноудалённости~--- к ближайшему чётному (roundTiesToEven по IEEE~754).
...
синтаксическим анализатором. Данные копируются через \texttt{memcpy}.
...
блоком (диспетчер блока (\texttt{rt\_stream.cpp})).

Математически: все записи \texttt{st.shared} до \texttt{bar.sync}
гарантированно видимы всем потокам после него.
Гергель и Стронгин~\cite[с.~142]{GergelStrongin2003} формулируют это требование так:
«изменение каким-либо потоком значений общих переменных должно приводить
к блокировке доступа к модифицируемым данным для всех остальных потоков».
Барьер обеспечивает именно это свойство~--- полную видимость памяти
(happens-before по отношению к записям разделяемой памяти).

Реализация барьера в эмуляторе зависит от режима сборки.
При компиляции с поддержкой OpenMP~\cite{Kornyakov2010} (\texttt{EMULATOR\_OPENMP\_ENABLED})
каждый варп блока исполняется в отдельном потоке ОС.
Технология OpenMP~\cite[с.~141]{GergelStrongin2003} использует директивы параллелизма
«для выделения в программе параллельных областей (parallel regions), в которых
последовательный исполняемый код может быть разделён на несколько раздельных
командных потоков (threads)».
В реализации эмулятора это достигается директивой \texttt{\#pragma omp parallel for};
\texttt{bar.sync} реализован через \texttt{BlockBarrier}~--- циклический барьер
на базе \texttt{std::mutex} и \texttt{std::condition\_variable}.
Варп, достигший барьера, блокируется до тех пор, пока все активные
варпы блока не придут к той же точке; последний варп вызывает
\texttt{WarpDone()}, снимая блокировку остальных.

При сборке без OpenMP применяется кооперативный планировщик
(\textit{round-robin}): на каждой итерации каждый варп исполняет
инструкции до ближайшего \texttt{bar.sync} включительно, после чего
управление передаётся следующему варпу.
Когда все варпы достигли барьера, они продолжают исполнение совместно.
Эта схема корректно воспроизводит семантику барьерной синхронизации
без параллелизма на уровне ОС.
