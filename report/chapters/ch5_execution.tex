...
Поскольку эмулятор исполняет потоки последовательно (не параллельно),
семантика барьера реализуется тривиально: все записи, выполненные до
\texttt{bar.sync} в одном потоке, уже завершены к моменту, когда
любой другой поток приступает к чтению. Тем не менее явная реализация
барьера через \texttt{syncBarrier()} сохраняется для корректного подсчёта
метрики числа синхронизаций в профилировщике.
\newpage
