...
Разделение фазы создания объектов (при разборе) и фазы исполнения (в цикле
диспетчера) обеспечивает строгое разделение ответственности: синтаксический анализатор отвечает
за синтаксическую корректность, а исполнитель~--- исключительно за
семантику вычислений. Такое разделение облегчает как тестирование компонентов,
так и расширение набора поддерживаемых инструкций.
...
единый JSON-манифест управляет генерацией кода сразу нескольких компонентов,
что исключает рассинхронизацию между синтаксическим анализатором, фабрикой и профилировочными
хуками при добавлении новых инструкций.
...
Унификация типов хранения в \texttt{uint64\_t} сокращает объём кода
регистрового файла и исключает шаблонный параметр по типу хранения
из структуры \texttt{ThreadContext}. Альтернативный подход с раздельными
векторами для каждого типа потребовал бы отдельных специализаций
для \texttt{float}, \texttt{double}, \texttt{int32\_t} и прочих,
что привело бы к значительному увеличению объёма кода без функциональных преимуществ.
...
Правила структурной операционной семантики~\cite{Plotkin1981} для ключевых целочисленных инструкций:
...
Явная обработка этих граничных случаев критически важна для достоверности
эмулятора: реальный GPU аппаратно следует семантике PTX-спецификации,
и отклонение результата в пограничных сценариях привело бы к расхождению
верифицируемых значений даже при корректной обработке типичных путей исполнения.
...
При компиляции с поддержкой OpenMP (\texttt{EMULATOR\_OPENMP\_ENABLED})
каждый варп блока исполняется в отдельном потоке ОС
(\texttt{\#pragma omp parallel for}), а \texttt{bar.sync} реализован
через \texttt{BlockBarrier}~--- циклический барьер на базе
\texttt{std::mutex} и \texttt{std::condition\_variable}.
